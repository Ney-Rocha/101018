...
class ProfessorTests(TestCase):
    ...
    def test_20_alterar_usuario(self):
        p = Professor()
        p.login = 'Leoshow'
        p.save()
        p.email = 'leoshow@email.com'
        p.save()
        p2 = Professor.objects.filter(id = p.id)
        self.assertEqual('leoshow@email.com',p2[0].email)
    
    '''
    Um aluno não pode fazer dois oferecimentos da mesma disciplina
    '''
    def test_21_disciplina_repetida(self):
        self.fail('nao implementado')
        self.prepara_para_matricula()
